    ...
    <div className="rounded-[10px] bg-white p-6">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <h3 className="mt-0.5 text-[15px] font-semibold text-[#1a1a1a]">{formatKey(rule.key)}</h3>
          {rule.description && (
          ...
          <div className="mt-4 flex items-center gap-3">
            <input
              ...
              className="flex-1 rounded-[10px] bg-[#f4f4f5] px-4 py-2.5 text-[14px] text-[#1a1a1a] outline-none transition-all focus:ring-2 focus:ring-[#1a1a1a]/10"
            />
            <button
              onClick={handleSave}
...
export function SettingsClient({ rules }: { rules: LoanRule[] }) {
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-8">
        <h2 className="text-[22px] font-extrabold tracking-[-0.03em] text-[#1a1a1a]">Settings</h2>
        ...
        {modelInfo ? (
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
            <div>
              ...
              <p className="mt-0.5 text-[14px] font-semibold text-[#1a1a1a]">v{modelInfo.version}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="mt-0.5 text-[14px] font-semibold text-[#1a1a1a]">{modelInfo.trainingSize}</p>
            </div>
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.05em] text-[#15803d]/60">Accuracy</p>
              <p className="mt-0.5 text-[14px] font-semibold text-[#1a1a1a]">{(modelInfo.accuracy * 100).toFixed(1)}%</p>
            </div>
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.05em] text-[#15803d]/60">Precision</p>
              <p className="mt-0.5 text-[14px] font-semibold text-[#1a1a1a]">{(modelInfo.precision * 100).toFixed(1)}%</p>
            </div>
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.05em] text-[#15803d]/60">Recall</p>
              <p className="mt-0.5 text-[14px] font-semibold text-[#1a1a1a]">{(modelInfo.recall * 100).toFixed(1)}%</p>
            </div>
          </div>
        ) : (
          <p className="text-[13px] text-[#71717a]">
            No risk model loaded. Run:{" "}
            <code className="rounded bg-white/60 px-1.5 py-0.5 font-mono text-[12px] text-[#1a1a1a]">
